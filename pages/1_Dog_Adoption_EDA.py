...
## 性別分析
tab_gender, tab_gender_bodytype = st.tabs(["公母比例", "公母 VS. 體型"])
with tab_gender:
    fig = dog_data_plotter.draw_pie(target_col="animal_sex", title="公母比例")
    md_text_list = ["- 整體公母差異並不大，分佈約是各半"]
    col_left_right_component(fig, md_text_list)

with tab_gender_bodytype:
    fig = dog_data_plotter.draw_pie(
        target_col="animal_sex", facet_col="animal_bodytype", title="公母 VS. 體型"
    )
...
with tabs[0]:
    fig = dog_data_plotter.draw_pie("animal_Variety_converted", title="品種比例")
    md_text_list = ["- 幾乎所有流浪狗的品種皆為米克斯"]
    col_left_right_component(fig, md_text_list)
with tabs[1]:
    fig = dog_data_plotter.dog_breed_exclude_mix()
...
tabs = st.tabs(tab_list)
with tabs[0]:
    fig = dog_data_plotter.draw_pie("animal_age", "年齡比例")
    md_text_list = ["- 流浪狗多數皆已是成犬"]
    col_left_right_component(fig, md_text_list)
    fig = dog_data_plotter.draw_pie(
        "animal_age", "體型 VS. 年齡", facet_col="animal_bodytype"
    )
    md_text_list = [
...
tabs = st.tabs(tab_list)
with tabs[0]:
    fig = dog_data_plotter.draw_pie("animal_colour", "毛色比例", color_by_target_col=False)
